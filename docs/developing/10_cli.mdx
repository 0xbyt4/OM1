...
**Options:**
- `--hot-reload, -r`: Trigger hot-reload only (touch config file)
- `--force, -f`: Force kill before restart
- `--timeout, -t`: Seconds to wait for shutdown (default: 5)
...
# Just trigger hot-reload (no full restart)
om1 restart ollama --hot-reload

# Force kill then restart
om1 restart ollama --force

# Custom timeout
om1 restart ollama --timeout 10
```
...
**Options:**
- `--mode-aware, -m`: Show only mode-aware configurations
- `--standard, -s`: Show only standard configurations
- `--format, -f`: Output format: table, simple, json (default: table)

**Examples:**
```bash
# List all configs (table format)
om1 list

# List only mode-aware configs
om1 list --mode-aware

# List only standard configs
om1 list --standard

# Output as JSON
om1 list --format json
...
```bash
om1 info <config_name> [OPTIONS]
```

**Options:**
- `--json, -j`: Output as JSON
- `--components, -c`: Show detailed component information
...
# Output as JSON
om1 info ollama --json

# Show detailed components
om1 info ollama --components

# Mode-aware config info
om1 info spot_modes
```
