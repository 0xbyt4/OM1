...
def test_recovery_callback_registered(base_url, api_key, fps, mock_dependencies):
    """Test that VLMGeminiProvider registers a recovery callback."""
    provider = VLMGeminiProvider(base_url, api_key, fps=fps)
    health = HealthMonitorProvider()

    assert "VLMGeminiProvider" in health._recovery_callbacks
    assert health._recovery_callbacks["VLMGeminiProvider"] == provider._recover
